...
export class BlockController {
  constructor(private readonly blockService: BlockService, private readonly userService: UserService) {}
  ...
  @Post()
  async block(@Body() blockCreateDto: BlockCreateDto, @Req() request: Request) {
  ...
  async remove(@Param("id") id: string, @Req() request: Request) {
    ...
    if (request.session.user_id !== block.sender.id) throw new BadRequestException("You can only remove block send by you)");
