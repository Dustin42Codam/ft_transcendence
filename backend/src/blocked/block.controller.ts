...
export class BlockController {
  constructor(private readonly blockService: BlockService, private readonly userService: UserService, private readonly authService: AuthService) {}
  ...
  @Get("user/:id")
  async getBlocksFromUser(@Param("id") id: string) {
  ...
  async block(@Param("id") receiverId: string, @Req() request: Request) {
    const senderId = await this.authService.userId(request);
  ...
  async remove(@Param("id") receiverId: string, @Req() request: Request) {
    ...
    if (block) this.blockService.delete(block.id);
  }
}
