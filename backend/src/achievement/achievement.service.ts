...
export class AchievementService extends AbstractService {
  ...
  async getAchievementById(id: number) {
    ...
    if (!achievement) throw new BadRequestException("This achievement does not exist");
    return achievement;
  }
  ...
  async checkFriendshipAchievement(user_id: number) {
    ...
    let i = 0;
    for (; i < user.achievements.length; i++) if (user.achievements[i].type == AchievementType.FRIENDS) break;
    const achievement = user.achievements[i];
    if (achievement.level === achievement.max_level) return;
  ...
  async checkGameAchievements(user_id: number) {
    const user = await this.userService.getUserById(user_id, ["achievements"]);
    let i = 0;
    for (; i < user.achievements.length; i++) if (user.achievements[i].type == AchievementType.GAMES_PLAYED) break;
    const achievementGamesPlayed = user.achievements[i];
    i = 0;
    for (; i < user.achievements.length; i++) if (user.achievements[i].type == AchievementType.GAMES_WON) break;
    const achievementGamesWon = user.achievements[i];

    if (
      achievementGamesPlayed.level === achievementGamesPlayed.max_level &&
    ...
    if (achievementGamesWon.level === achievementGamesWon.max_level && user.game_stats.win > achievements[achievementGamesWon.type].level_border[achievementGamesWon.level]) {
  ...
  async checkAddedAvatarAchievement(user_id: number) {
    const user = await this.userService.getUserById(user_id, ["achievements"]);
    let i = 0;
    for (; i < user.achievements.length; i++) if (user.achievements[i].type == AchievementType.ADDED_AVATAR) break;
    const achievement = user.achievements[i];
