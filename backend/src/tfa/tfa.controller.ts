...
export class TFAController {
  ...
  @HttpCode(200)
  @UseGuards(AuthGuard)
  async turnOffTwoFactorAuthentication(@Req() request: Request, @Body() { code }: tfaCodeDto) {
    const userId = await this.authService.userId(request);
    const user = await this.userService.getUserById(userId);
    const isCodeValid = this.tfaService.isTwoFactorAuthenticationCodeValid(code, user);
    if (!isCodeValid) {
  ...
  @HttpCode(200)
  @UseGuards(AuthGuard)
  async turnOnTwoFactorAuthentication(@Req() request: Request, @Body() { code }: tfaCodeDto) {
    const userId = await this.authService.userId(request);
    const user = await this.userService.getUserById(userId);
    const isCodeValid = this.tfaService.isTwoFactorAuthenticationCodeValid(code, user);
    if (!isCodeValid) {
  ...
  @HttpCode(200)
  @UseGuards(AuthGuard)
  async authenticate(@Req() request: Request, @Body() { code }: tfaCodeDto, @Res({ passthrough: true }) response: Response) {
    const userId = await this.authService.userId(request);
    const user = await this.userService.getUserById(userId);
