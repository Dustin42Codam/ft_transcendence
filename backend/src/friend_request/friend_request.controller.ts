...
export class FriendRequestController {
  ...
  @Post()
  async friendRequest(@Body() friendRequestCreateDto: FriendRequestCreateDto, @Req() request: Request) {
    ...
    const friendRequest = await this.friendRequestService.find({
      where: [{ sender: sender.id }, { receiver: friendRequestCreateDto.receiver }],
    });
    ...
    if (blockBySender) throw new BadRequestException("You can not send a friendRequest to a User that you blocked.");
    const blockByReceiver = await this.blockService.findOne({
      sender: friendRequestCreateDto.receiver,
      receiver: sender,
    });
    if (blockByReceiver) throw new BadRequestException("You can not send a friendRequest to a User that blocked you.");
    const friendRequestFromReceiver = await this.friendRequestService.findOne({
      sender: friendRequestCreateDto.receiver,
      receiver: sender,
    });
    if (friendRequestFromReceiver) throw new BadRequestException("The user you want to send a friend request to already send a friendrequest to you.");
    const friendship = await this.friendService.getFriendshipByUserids(friendRequestCreateDto.receiver.id, sender.id);
    if (friendship) throw new BadRequestException("You are already friends with this user.");
    return await this.friendRequestService.createFriendRequest(friendRequestCreateDto);
  }

  @Post("accept/:id")
  async acceptFriendRequest(@Param("id") id: string, @Req() request: Request) {
    const friendRequest = await this.friendRequestService.getFriendRequestById(Number(id));
    if (request.session.user_id !== friendRequest.receiver.id) throw new BadRequestException("You can only accept friend request that where send to you");
    return await this.friendRequestService.acceptFriendRequest(friendRequest);
  }

  @Post("decline/:id")
  async declineFriendRequest(@Param("id") id: string, @Req() request: Request) {
    const friendRequest = await this.friendRequestService.getFriendRequestById(Number(id));
    if (request.session.user_id !== friendRequest.receiver.id) throw new BadRequestException("You can only accept friend request that where send to you");
  ...
  @Post("remove/:id")
  async removeSendFriendRequest(@Param("id") id: string, @Req() request: Request) {
    const friendRequest = await this.friendRequestService.getFriendRequestById(Number(id));
    if (request.session.user_id !== friendRequest.sender.id) throw new BadRequestException("You can only remove friend request that you send");
    return this.friendRequestService.delete(Number(id));
  }
}
