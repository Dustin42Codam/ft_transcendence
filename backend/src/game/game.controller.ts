...
export class GameController {
  constructor(private readonly gameService: GameService, private readonly authServcie: AuthService, private readonly userService: UserService) {}
  ...
  async joinPrivateGameByInviteCode(@Param("invite_code") invite_code: string, @Req() request: Request) {
  ...
  @Post("classic")
  async joinClassicGame(@Req() request: Request) {
    const userId = await this.authServcie.userId(request);
    const user = await this.userService.getUserById(userId);
    if (await this.gameService.isAlreadyInGame(user)) throw new BadRequestException("This user is already in a game");
  ...
  @Post("power_up")
  async joinPowerUpGame(@Req() request: Request) {
    const userId = await this.authServcie.userId(request);
    const user = await this.userService.getUserById(userId);
    if (await this.gameService.isAlreadyInGame(user)) throw new BadRequestException("This user is already in a game");
  ...
  @Post("leave/id/:id")
  async end(@Param("id") id: string, @Req() request: Request) {
    const userId = await this.authServcie.userId(request);
    const user = await this.userService.getUserById(userId);
