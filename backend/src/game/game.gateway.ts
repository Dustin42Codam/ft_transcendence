...
export class GameGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
  ...
  async physicLoop(activeGames: Array<GameRoom>, logger: any, io: any, gameService: GameService): Promise<void> {
    function getRandomPowerUp(): PowerUp {
      return {
        positionX: Math.floor( 100 + Math.random() * 1099),
        positionY: Math.floor( 100 + Math.random() * 499),
        //positionX: 650,DEBUG
        //positionY: 350,DEBUG
        width: 40,
        height: 40,
    ...
    function doesOverlapWith(ball: Ball, obj: any) {
      if (ball.positionX > obj.positionX + obj.width || obj.positionX > ball.positionX + ball.width) return false;
      if (ball.positionY + ball.height < obj.positionY || obj.positionY + obj.height < ball.positionY) return false;
      return true;
    }
          ...
          if (gameHasStarted(game)) {
            ...
            if (isPowerUp(game.gamePhysics.powerUp)) {
              if (
                ...
                !powerUpOnBoard(game) &&
                (game.gamePhysics.player1.bat.lastTouch || game.gamePhysics.player2.bat.lastTouch)
              ) {
            ...
            if (checkIfScore(game)) {
              ...
              gameService.addScore(Number(game.gameRoomId), game.gamePhysics.score[0], game.gamePhysics.score[1]).then(be_game => {
  ...
  async handelLeaveRoom(client: Socket, payload: any): Promise<void> {
    ...
    if (currentActiveGame) {
      if (
        ...
        JSON.stringify(currentActiveGame.gamePhysics.player2) == JSON.stringify(defaultPlyaer)
      ) {
  ...
  async handelJoinRoom(client: Socket, gameRoomId: string): Promise<void> {
    ...
    if (!gameFromDb) throw "game with id not found";
