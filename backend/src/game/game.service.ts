...
export class GameService extends AbstractService {
  constructor(private gameStatsService: GameStatsService, private userService: UserService, @InjectRepository(Game) private readonly gameRepository: Repository<Game>) {
  ...
  async isAlreadyInGame(user: User) {
    const allPendingGames: Game[] = await this.gameRepository.find({
      where: [{ status: GameStatus.PENDING }, { status: GameStatus.ACTIVE }],
    });
